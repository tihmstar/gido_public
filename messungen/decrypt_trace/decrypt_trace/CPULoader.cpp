...
#define MAX_QUEUE_CNT 16

CPULoader::CPULoader(const std::vector<std::string> &toLoadList, ssize_t maxRamAlloc, uint16_t maxLoaderThreads, std::function<bool(LoadTraceFile*)> filterfunc)
: _toLoadList{toLoadList}, _runningWorkersCnt(0), _filesCnt(0), _maxRamAlloc(maxRamAlloc), _filterfunc(filterfunc)
{
    if (!maxLoaderThreads) {
...
void CPULoader::worker(int loaderNum){
    ...
    while (_toLoadList.size()) {
        ...
        std::string LoadTraceFilename = _toLoadList.back();
        _toLoadList.pop_back();
        printf("[L-%02d] (%3lu of %3lu) loading=%s\n",loaderNum,_filesCnt-_toLoadList.size(),_filesCnt,LoadTraceFilename.c_str());
        _listLock.unlock();

        LoadTraceFile *lf = new LoadTraceFile(LoadTraceFilename.c_str(), false);
        
        if (_filterfunc) {
            if (!_filterfunc(lf)){
                printf("[L-%02d] skipped=%s\n",loaderNum,LoadTraceFilename.c_str());
                delete lf;
                continue;
            }
        }
        ...
        while (lf->fsize() > _maxRamAlloc || (_loadedQueue.size() > MAX_QUEUE_CNT)) {//access happens while we have lock
        ...
        //update size
        _maxRamAlloc.fetch_sub(lf->fsize());
...
LoadTraceFile *CPULoader::dequeue(){
    LoadTraceFile *ret = NULL;
    while (true){
        _queueLock.lock();
