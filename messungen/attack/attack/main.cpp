...
int doAttack(const char *indir, const char *outfile, int threadsCnt, int maxTraces){
    ...
    float gmean[point_per_trace];
    float gcensum[point_per_trace];


    memset(gmean, 0, point_per_trace * sizeof(float));
    memset(gcensum, 0, point_per_trace * sizeof(float));


    float ghypotMeanGuessVals[0x100] = {};
    float ghypotCensumGuessVals[0x100] = {};
    ...
    for (int k = 0; k<0x100; k++) {
        gupperPart[k].cnt = 0;
        gupperPart[k].trace = (float*)malloc(point_per_trace * sizeof(float));
        memset(gupperPart[k].trace, 0, point_per_trace * sizeof(float));
    }
    ...
    auto workerfunc = [&](int workerNum)->void{
        ...
        while (true) {
            ...
            if (toNormalizeList.size()) {
                ...
                float curhypotMeanGuessVals[0x100] = {};
                uint64_t curhypotMeanGuessCnt[0x100] = {};

                float curhypotCensumGuessVals[0x100] = {};
                ...
                for (int k = 0; k<0x100; k++) {
                    ...
                    curupperPart[k].trace = (float*)malloc(point_per_trace * sizeof(float));
                    memset(curupperPart[k].trace, 0, point_per_trace * sizeof(float));
                }
                ...
                float *curmean = (float *)malloc(point_per_trace * sizeof(float));
                float *curcensum = (float *)malloc(point_per_trace * sizeof(float));

                memset(curmean, 0, point_per_trace * sizeof(float));
                memset(curcensum, 0, point_per_trace * sizeof(float));
...
//        workerfunc(i); //for debugging
    ...
    for (int k=0; k<0x100; k++) {
        ...
        myfile out(filename.c_str(),point_per_trace*sizeof(float));
        memset(&out[0], 0, point_per_trace*sizeof(float));
        
        float *upTrace = gupperPart[k].trace;
        float hypotCensum = ghypotCensumGuessVals[k];
        for (int i=0; i<point_per_trace; i++) {
            float upper = upTrace[i];
            
            float realCensum = gcensum[i];
            
            float preLower = realCensum * hypotCensum;
            
            float lower = sqrt(preLower);
