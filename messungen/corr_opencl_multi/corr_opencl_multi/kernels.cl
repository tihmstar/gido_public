...
/* BEGIN BLOCK 0 */
inline uchar powerModel_DEC_INPUT(__global const struct MeasurementStructure *msm){
    return msm->Plaintext[G_SELECTOR_VALUE];
}

inline uchar powerModel_DEC_INPUT_HW(__global const struct MeasurementStructure *msm){
    return hamming_weight(msm->Plaintext[G_SELECTOR_VALUE]);
}

inline uchar powerModel_DEC_INPUT_HW4(__global const struct MeasurementStructure *msm){
    return hamming_weight_32(*(__global uint*)(&msm->Plaintext[G_SELECTOR_VALUE]));
}

inline uchar powerModel_DEC_INPUT_KEY_HW(__global const struct MeasurementStructure *msm){
    return hamming_weight(msm->Plaintext[G_SELECTOR_VALUE] ^ aeskey[16*14 + G_SELECTOR_VALUE]);
}

inline uchar powerModel_SBOX(__global const struct MeasurementStructure *msm){
    return inv_sbox[msm->Plaintext[G_SELECTOR_VALUE] ^ aeskey[16*14 + G_SELECTOR_VALUE]];
}

inline uchar powerModel_SBOX_HW(__global const struct MeasurementStructure *msm){
    return hamming_weight(inv_sbox[msm->Plaintext[G_SELECTOR_VALUE] ^ aeskey[16*14 + G_SELECTOR_VALUE]]); //hamming sbox
}

inline uchar powerModel_ZEROVALUE(__global const struct MeasurementStructure *msm){
    return (msm->Plaintext[G_SELECTOR_VALUE] ^ aeskey[16*14 + G_SELECTOR_VALUE]) == 0; //sbox input ZeroValue
}

inline uchar powerModel_DEC_OUTPUT(__global const struct MeasurementStructure *msm){
    return msm->Ciphertext[G_SELECTOR_VALUE];
}

inline uchar powerModel_DEC_OUTPUT_HW(__global const struct MeasurementStructure *msm){
    return hamming_weight(msm->Ciphertext[G_SELECTOR_VALUE]);
}

inline uchar powerModel_DEC_OUTPUT_HW4(__global const struct MeasurementStructure *msm){
    return hamming_weight_32(*(__global uint*)(&msm->Ciphertext[G_SELECTOR_VALUE]));
}
/* END BLOCK 0 */

inline uchar powerModel_TTABLET0_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ptextAddkey[ 3] = msm->Plaintext[ 3] ^ aeskey[16*14 +  3];
    ptextAddkey[ 4] = msm->Plaintext[ 4] ^ aeskey[16*14 +  4];
    ptextAddkey[ 5] = msm->Plaintext[ 5] ^ aeskey[16*14 +  5];
    ptextAddkey[ 6] = msm->Plaintext[ 6] ^ aeskey[16*14 +  6];
    ptextAddkey[ 7] = msm->Plaintext[ 7] ^ aeskey[16*14 +  7];
    ptextAddkey[ 8] = msm->Plaintext[ 8] ^ aeskey[16*14 +  8];
    ptextAddkey[ 9] = msm->Plaintext[ 9] ^ aeskey[16*14 +  9];
    ptextAddkey[10] = msm->Plaintext[10] ^ aeskey[16*14 + 10];
    ptextAddkey[11] = msm->Plaintext[11] ^ aeskey[16*14 + 11];
    ptextAddkey[12] = msm->Plaintext[12] ^ aeskey[16*14 + 12];
    ptextAddkey[13] = msm->Plaintext[13] ^ aeskey[16*14 + 13];
    ptextAddkey[14] = msm->Plaintext[14] ^ aeskey[16*14 + 14];
    ptextAddkey[15] = msm->Plaintext[15] ^ aeskey[16*14 + 15];
    return hamming_weight_32(T0_inv[ptextAddkey[0]]); //hamming T0
}

inline uchar powerModel_TTABLET1_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    ptextAddkey[14] = msm->Plaintext[14] ^ aeskey[16*14 + 14];
    ptextAddkey[15] = msm->Plaintext[15] ^ aeskey[16*14 + 15];
    return hamming_weight_32(T1_inv[ptextAddkey[13]]); //hamming T1
}

inline uchar powerModel_TTABLET2_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    ptextAddkey[14] = msm->Plaintext[14] ^ aeskey[16*14 + 14];
    ptextAddkey[15] = msm->Plaintext[15] ^ aeskey[16*14 + 15];
    return hamming_weight_32(T2_inv[ptextAddkey[10]]); //hamming T2
}

inline uchar powerModel_TTABLET3_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    ptextAddkey[14] = msm->Plaintext[14] ^ aeskey[16*14 + 14];
    ptextAddkey[15] = msm->Plaintext[15] ^ aeskey[16*14 + 15];
    return hamming_weight_32(T3_inv[ptextAddkey[ 7]]); //hamming T3
}

inline uchar powerModel_TTABLET0T1_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    ptextAddkey[14] = msm->Plaintext[14] ^ aeskey[16*14 + 14];
    ptextAddkey[15] = msm->Plaintext[15] ^ aeskey[16*14 + 15];
    return hamming_weight_32(T0_inv[ptextAddkey[0]] ^ T1_inv[ptextAddkey[13]]);
}

inline uchar powerModel_TTABLET0T1T2_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    ptextAddkey[14] = msm->Plaintext[14] ^ aeskey[16*14 + 14];
    ptextAddkey[15] = msm->Plaintext[15] ^ aeskey[16*14 + 15];
    return hamming_weight_32(T0_inv[ptextAddkey[0]] ^ T1_inv[ptextAddkey[13]] ^ T2_inv[ptextAddkey[10]]);
}

inline uchar powerModel_TTABLET0T1T2T3_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    ptextAddkey[14] = msm->Plaintext[14] ^ aeskey[16*14 + 14];
    ptextAddkey[15] = msm->Plaintext[15] ^ aeskey[16*14 + 15];
    return hamming_weight_32(T0_inv[ptextAddkey[0]] ^ T1_inv[ptextAddkey[13]] ^ T2_inv[ptextAddkey[10]] ^ T3_inv[ptextAddkey[ 7]]);
}

inline uchar powerModel_TTABLET0P0_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    ptextAddkey[14] = msm->Plaintext[14] ^ aeskey[16*14 + 14];
    ptextAddkey[15] = msm->Plaintext[15] ^ aeskey[16*14 + 15];
    return hamming_weight_32(T0_inv[ptextAddkey[0]] ^ *(__global uint*)(&msm->Plaintext));
}

inline uchar powerModel_TTABLET0P0K0_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    ptextAddkey[14] = msm->Plaintext[14] ^ aeskey[16*14 + 14];
    ptextAddkey[15] = msm->Plaintext[15] ^ aeskey[16*14 + 15];
    return hamming_weight_32(T0_inv[ptextAddkey[0]] ^ *(uint*)(ptextAddkey));
}

inline uchar powerModel_P0K0_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    ptextAddkey[14] = msm->Plaintext[14] ^ aeskey[16*14 + 14];
    ptextAddkey[15] = msm->Plaintext[15] ^ aeskey[16*14 + 15];
    return hamming_weight_32(*(uint*)(ptextAddkey));
}

inline uchar powerModel_ROUND_HW(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    return hamming_weight_32(*(uint*)(&ptextAddkey[G_SELECTOR_VALUE]));
}

inline uchar powerModel_ROUND_HW128(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_HW64_0(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_HW64_1(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_HW32_0(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_HW32_1(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_HW16_0(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_HW16_1(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_XOR_NEXT_HW128(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    for (int i=1; i<G_SELECTOR_ROUND+1; i++) { //4 middle rounds
        ...
        if (i == G_SELECTOR_ROUND){ //this is last round
            if (i == 14){
                ...
                ptextAddkey[ 0] ^= msm->Ciphertext[ 0];
                ptextAddkey[ 1] ^= msm->Ciphertext[ 1];
                ptextAddkey[ 2] ^= msm->Ciphertext[ 2];
                ptextAddkey[ 3] ^= msm->Ciphertext[ 3];
                ptextAddkey[ 4] ^= msm->Ciphertext[ 4];
                ptextAddkey[ 5] ^= msm->Ciphertext[ 5];
                ptextAddkey[ 6] ^= msm->Ciphertext[ 6];
                ptextAddkey[ 7] ^= msm->Ciphertext[ 7];
                ptextAddkey[ 8] ^= msm->Ciphertext[ 8];
                ptextAddkey[ 9] ^= msm->Ciphertext[ 9];
                ptextAddkey[10] ^= msm->Ciphertext[10];
                ptextAddkey[11] ^= msm->Ciphertext[11];
                ptextAddkey[12] ^= msm->Ciphertext[12];
                ptextAddkey[13] ^= msm->Ciphertext[13];
                ptextAddkey[14] ^= msm->Ciphertext[14];
                ptextAddkey[15] ^= msm->Ciphertext[15];
#else
                ptextAddkey[ 0] ^= msm->Ciphertext[ 0] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 + 0];
                ptextAddkey[ 1] ^= msm->Ciphertext[ 1] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 + 1];
                ptextAddkey[ 2] ^= msm->Ciphertext[ 2] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 + 2];
                ptextAddkey[ 3] ^= msm->Ciphertext[ 3] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 + 3];
                ptextAddkey[ 4] ^= msm->Ciphertext[ 4] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 + 4];
                ptextAddkey[ 5] ^= msm->Ciphertext[ 5] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 + 5];
                ptextAddkey[ 6] ^= msm->Ciphertext[ 6] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 + 6];
                ptextAddkey[ 7] ^= msm->Ciphertext[ 7] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 + 7];
                ptextAddkey[ 8] ^= msm->Ciphertext[ 8] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 + 8];
                ptextAddkey[ 9] ^= msm->Ciphertext[ 9] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 + 9];
                ptextAddkey[10] ^= msm->Ciphertext[10] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 +10];
                ptextAddkey[11] ^= msm->Ciphertext[11] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 +11];
                ptextAddkey[12] ^= msm->Ciphertext[12] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 +12];
                ptextAddkey[13] ^= msm->Ciphertext[13] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 +13];
                ptextAddkey[14] ^= msm->Ciphertext[14] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 +14];
                ptextAddkey[15] ^= msm->Ciphertext[15] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 +15];
#endif
            }else{
                *(uint*)&ptextAddkey[ 0] ^= w[0];
                *(uint*)&ptextAddkey[ 4] ^= w[1];
...
inline uchar powerModel_ROUND_XOR_NEXT_HW64_0(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    return hamming_weight_p64(ptextAddkey);
}

inline uchar powerModel_ROUND_XOR_NEXT_HW64_1(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_XOR_NEXT_HW32_0(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_XOR_NEXT_HW32_1(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_XOR_NEXT_HW32_2(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_XOR_NEXT_HW32_3(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_XOR_NEXT_HW16_0(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_XOR_NEXT_HW16_1(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_XOR_NEXT_HW16_2(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    return hamming_weight_p16(&ptextAddkey[4]);
}

inline uchar powerModel_ROUND_XOR_NEXT_HW16_3(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    return hamming_weight_p16(&ptextAddkey[6]);
}

#define BYTE_SELECTOR 0
inline uchar powerModel_ROUND_XOR_NEXT_HW8_BYTE_SELECTOR(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    return hamming_weight(ptextAddkey[BYTE_SELECTOR]);
}

inline uchar powerModel_ROUND_XOR_NEXT_byte0_HW1_BYTE_SELECTOR(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    return (ptextAddkey[0] >> BYTE_SELECTOR) & 1;
}

inline uchar powerModel_ROUND_XOR_NEXT_zeroval_BYTE_SELECTOR(__global const struct MeasurementStructure *msm){
#if AES_BLOCK_SELECTOR == 0
#   define plaintext(i) (msm->Ciphertext[ i])
#else
#   define plaintext(i) (msm->Ciphertext[ i] ^ msm->_shiftp[AES_BLOCK_SELECTOR*16 -16 + i])
#endif
    return (aeskey[BYTE_SELECTOR] ^ plaintext(BYTE_SELECTOR)) == 0;
}

inline uchar powerModel_ROUND_XOR_NEXT_MC_HW128(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_XOR_NEXT_SBOX_HW128(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    for (int i=1; i<G_SELECTOR_ROUND+1; i++) { //4 middle rounds
        ...
        if (i == G_SELECTOR_ROUND){ //this is last round
            for (int i=0; i<16; i++){
                ptextAddkey[i] = inv_sbox[ptextAddkey[i]];
                ((uchar*)w)[i] = inv_sbox[((uchar*)w)[i]];
            }
            *(uint*)&ptextAddkey[ 0] ^= w[0];
            *(uint*)&ptextAddkey[ 4] ^= w[1];
            *(uint*)&ptextAddkey[ 8] ^= w[2];
...
inline uchar powerModel_ROUND_VALUE(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    return ptextAddkey[G_SELECTOR_VALUE];
}

inline uchar powerModel_ROUND_SBOX(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    return inv_sbox[ptextAddkey[G_SELECTOR_VALUE]];
}

inline uchar powerModel_ROUND_SBOX_HW(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    return hamming_weight(inv_sbox[ptextAddkey[G_SELECTOR_VALUE]]);
}

inline uchar powerModel_ROUND_SBOX_HW128(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_NEXT_SBOX_HW128(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_MC(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_MC_HW(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_MC_HW128(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar powerModel_ROUND_NEXT_MC_HW128(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
__kernel void computeMean(__global const struct Tracefile *tr, __global double *output, __global ulong *curcntMean){
    ...
    output[i] = 0; //clear buffer before access
...
__kernel void computeMeanHypot(__global const struct Tracefile *tr, __global double *meanHypot, __global ulong *meanHypotCnt){
    ...
    if (k>=1) return; //these guys are out of bounds

    uint nextMsmOffset = sizeof(struct MeasurementStructure)+pointsPerTrace;

    double factor = (1.0/tracesInFile);
    
    meanHypot[k] = 0;//clean buffer before access
    
    __global const struct MeasurementStructure *msm = tr->ms;
    for (uint j=0; j < tracesInFile; j++,msm = (__global const struct MeasurementStructure *)(((__global uchar*)msm)+nextMsmOffset)){

        uchar hypot = powerModel(msm);
        meanHypot[k] += hypot * factor;
    }
...
__kernel void computeCenteredSum(__global const struct Tracefile *tr, __global const double *mean, __global const double *meansHypot, __global double *censum, __global ulong *censumCnt){
    ...
    censum[i] = 0;//clean buffer before access
...
__kernel void computeUpper(__global const struct Tracefile *tr, __global const double *mean, __global const double *meansHypot, __global double *upperPart){
    ...
    upperPart[i] = 0;//clean buffer before access
    ...
    for (uint j=0; j < tracesInFile; j++,msm = (__global const struct MeasurementStructure *)(((__global uchar*)msm)+nextMsmOffset)){
        ...
        {
            uchar hypot = powerModel(msm);
            double hypotTmp = hypot - meansHypot[0];
            upperPart[i] += hypotTmp*tmp;
        }
    }
}
...
__kernel void computeCenteredSumHypot(__global const struct Tracefile *tr, __global const double *meansHypot, __global double *censumHypot, __global ulong *censumHypotCnt){
    ...
    censumHypot[k] = 0;//clean buffer before access
    ...
    for (uint j=0; j < tracesInFile; j++,msm = (__global const struct MeasurementStructure *)(((__global uchar*)msm)+nextMsmOffset)){
        ...
        //hypot
        uchar hypot = powerModel(msm);
        double hypotTmp = hypot - meansHypot[k];
        
        censumHypot[k] += hypotTmp*hypotTmp;
    }
