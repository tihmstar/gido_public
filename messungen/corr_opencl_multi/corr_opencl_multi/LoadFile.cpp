...
static std::atomic<ssize_t> gmaxRamAlloc{(ssize_t)10e9};
...
void LoadFile::load(){
    ...
    //    mlock(_fmem, _fsize); //don't care if this fails
        ...
        if (r == ARCHIVE_OK){
            ...
            assure(didExtract); // if this is detected as tar.gz, then non extracting the content is considered an error
        }
