    ...
    _deviceMemAlloced(0),_GpuFreeMem(0),_activeTraces(0),_processedTraceFiles(0),_workersWaitingForMemoryCnt(0), _workersOnGPUCnt(0)
{
    ...
    for (auto &m : models) {
        uint32_t modelsize = 0x100;
        
        if (m.find("powerModel_ATK") == std::string::npos) {
            printf("[D-%d] Found model in non-attack mode, setting model size to 1!\n",_deviceNum);
            modelsize = 1;
        }
        
        _gpuPowerModels.push_back(new GPUPowerModel{this,_deviceNum, _device_id, _context, kernelsource, point_per_trace, m, modelsize});
    }
