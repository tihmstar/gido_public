...
: _fd(-1),_fmem(NULL), _path(path)
{
    ...
    if (strncmp(_path.c_str(), "http", strlen("http")) == 0) {
        ...
        double dlen = 0;
        curl_easy_getinfo(mcurl, CURLINFO_CONTENT_LENGTH_DOWNLOAD, &dlen);
    ...
    gmaxRamModLock.lock();usleep(100);
    while (gmaxRamAlloc.fetch_sub(_fsize) < _fsize) {//access happens while we have lock
        ...
        gmaxRamModEventLock.lock();usleep(100); //hang here
        
        gmaxRamModLock.lock();usleep(100);//re-grab lock before re-read
    }
