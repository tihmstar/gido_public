...
GPUPowerModel::GPUPowerModel(GPUDevice *parent, int deviceNum, cl_device_id device_id, cl_context context, std::string kernelsource, uint32_t point_per_trace, std::string modelname, uint32_t modelSize)
: _modelSize(modelSize), _parent(parent), _deviceNum(deviceNum), _device_id(device_id), _context(context), _point_per_trace(point_per_trace), _modelname(modelname), _deviceWorkgroupSize(0), _program(NULL), _command_queue(NULL)
,_deviceMemAlloced(0)
...
,_gDeviceUpperPart(NULL)
{
    ...
    {
        ssize_t blockByteStrPos = modelname.find("_");
        std::string blockselstr = modelname.substr(0,blockByteStrPos+1);
        if (isdigit(blockselstr.c_str()[0])){
            modelname = modelname.substr(blockByteStrPos+1);

            blockselector = atoi(blockselstr.c_str());
            
            
            std::string oldKeySelector = "#define AES_BLOCK_SELECTOR";
            ssize_t oldKeySelectorPos = kernelsource.find(oldKeySelector);
            std::string newKeySelector = "#define AES_BLOCK_SELECTOR ";
            newKeySelector += std::to_string(blockselector);
            newKeySelector += " //";
    ...
    {
        std::string oldKeySelector = "#define KEY_GUESS_RANGE";
        ssize_t oldKeySelectorPos = kernelsource.find(oldKeySelector);
        std::string newKeySelector = "#define KEY_GUESS_RANGE ";
        newKeySelector += std::to_string(_modelSize);
        newKeySelector += " //";
...
GPUPowerModel::~GPUPowerModel(){
    ...
    while (_tracesQueue.size() > 0) {
        ...
        _tracesQueueInProcessLockEvent.lock();usleep(100);
    }
...
//    printf("[D-%u][M-%s] enqueueTrace\n",_deviceNum,_modelname.c_str());
    ...
    _tracesQueueLock.lock();usleep(100); //grab lock for modifying queue
...
void GPUPowerModel::getResults(std::map<std::string,CPUModelResults*> &modelresults){
    cl_uint clret = 0;
