...
#define MAX_QUEUE_CNT 30

CPULoader::CPULoader(const std::vector<std::string> &toLoadList, ssize_t maxRamAlloc, uint16_t maxLoaderThreads)
: _toLoadList{toLoadList}, _runningWorkersCnt(0), _filesCnt(0), _maxRamAlloc(maxRamAlloc)
{
    if (!maxLoaderThreads) {
        maxLoaderThreads = sysconf(_SC_NPROCESSORS_ONLN);
        
        if (maxLoaderThreads > 8) {
            //Having more than 8 loaders usually decreases initial performance
            maxLoaderThreads = 8;
        }
    }
...
void CPULoader::worker(int loaderNum){
    ...
    while (_toLoadList.size()) {
        ...
        LoadFile *lf = new LoadFile(loadFilename.c_str(), false);
        
        _maxRamModLock.lock();usleep(100); //grab lock for read/write
        while (lf->size() > (ssize_t)_maxRamAlloc || (_loadedQueue.size() > MAX_QUEUE_CNT)) {//access happens while we have lock
            _maxRamModLock.unlock();//release lock befor hang

            printf("[L-%02d] hanging until free event (queuesize=%lu)\n",loaderNum,_loadedQueue.size());
            _maxRamModEventLock.lock();usleep(100); //hang here
            
            _maxRamModLock.lock();usleep(100);//re-grab lock before re-read
        }
        oldsize = lf->size();
        _maxRamAlloc.fetch_sub(lf->size());
        ...
        //update size
        _maxRamAlloc.fetch_sub(lf->size());
        _maxRamAlloc.fetch_add(oldsize);

        
        _queueLock.lock();usleep(100);
...
LoadFile *CPULoader::dequeue(){
    ...
    while (true){
        ...
        if (_loadedQueue.size()){
            ...
            _maxRamModLock.lock();usleep(100); //grab lock for read/write
            _maxRamAlloc.fetch_add(ret->size());
        ...
        _queueAppendEventLock.lock();usleep(100); //wait
    }
