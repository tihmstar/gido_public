...
, _kernel_combineUpper(NULL)
{
    ...
    */
    ...
    /*
    gpu_float_type curhypotMeanGuessVals[_parent->_modelSize] = {};
    gpu_float_type curhypotCensumGuessVals[_parent->_modelSize] = {};
    */

    _curHypotMeans = clCreateBuffer(_parent->_context, CL_MEM_READ_WRITE, _parent->_modelSize * sizeof(gpu_float_type), NULL, &clret);assure(!clret);
    _parent->_deviceMemAlloced += _parent->_modelSize * sizeof(gpu_float_type);

    _curHypotCensums = clCreateBuffer(_parent->_context, CL_MEM_READ_WRITE, _parent->_modelSize * sizeof(gpu_float_type), NULL, &clret);assure(!clret);
    _parent->_deviceMemAlloced += _parent->_modelSize * sizeof(gpu_float_type);


    /*
     uint64_t curhypotMeanGuessCnt[_parent->_modelSize] = {};
     uint64_t curhypotCensumGuessCnt[_parent->_modelSize] = {};
     */
    _curHypotMeansCnt = clCreateBuffer(_parent->_context, CL_MEM_READ_WRITE, _parent->_modelSize * sizeof(cl_ulong), NULL, &clret);assure(!clret);
    _parent->_deviceMemAlloced += _parent->_modelSize * sizeof(cl_ulong);

    _curHypotCensumsCnt = clCreateBuffer(_parent->_context, CL_MEM_READ_WRITE, _parent->_modelSize * sizeof(cl_ulong), NULL, &clret);assure(!clret);
    _parent->_deviceMemAlloced += _parent->_modelSize * sizeof(cl_ulong);
    
    /*
     numberedTrace curgupperPart = (numberedTrace *)malloc(_parent->_modelSize*sizeof(numberedTrace));
     for (int k = 0; k<_parent->_modelSize; k++) {
     ...
     */
    _curUpperPart = clCreateBuffer(_parent->_context, CL_MEM_READ_WRITE, _parent->_modelSize * _parent->_point_per_trace * sizeof(gpu_float_type), NULL, &clret);assure(!clret);
    ...
    uint32_t modelSize = _parent->_modelSize;
    ...
    clret = clSetKernelArg(_kernel_combineMeanAndCenteredSumHypot, 8, sizeof(cl_uint), &modelSize);assure(!clret);
    ...
    clret = clSetKernelArg(_kernel_mergeCoutnersHypot, 4, sizeof(cl_uint), &modelSize);assure(!clret);

    
    cl_uint valUpperTotal = modelSize * _parent->_point_per_trace;
...
void GPUPowerModelExecutionWorker::waitForMyPrevEventCompletion(){
    cl_int clret = 0;
    _prevEventAccessLock.lock();usleep(100);
...
void GPUPowerModelExecutionWorker::worker(){
    ...
    global_work_size_key[1] = (_parent->_modelSize / _parent->_deviceWorkgroupSize);
    if (_parent->_modelSize % _parent->_deviceWorkgroupSize != 0) global_work_size_key[1] ++;
                
    size_t global_work_size_upper[2] = {};
    global_work_size_upper[0] = _parent->_deviceWorkgroupSize;
    global_work_size_upper[1] = ((_parent->_point_per_trace*_parent->_modelSize) / _parent->_deviceWorkgroupSize);
    if ((_parent->_point_per_trace*_parent->_modelSize) % _parent->_deviceWorkgroupSize != 0) global_work_size_upper[1] ++;
    ...
    while (true) {
        ...
        if (_parent->_tracesQueue.size() == 0){
            _parent->_tracesQueueLockEvent.lock();usleep(100); //wait for event
        }
        
        _parent->_tracesQueueLock.lock();usleep(100); //grab lock for modiying queue
...
#warning We possibly don't need (want?) this clFlush
...
cl_int GPUPowerModelExecutionWorker::relaxedWaitForEvent(cl_uint num_events, const cl_event * event_list){
    ...
    delta = dend - dstart;
    _kern_avg_runLock.lock();usleep(100);
