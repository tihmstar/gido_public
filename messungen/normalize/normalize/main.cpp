...
//#define SAMPLE_SIZE 20000
                ...
                afterfilter1:
                    ...
                    for (int p=0; p<4000; p++) {
                        if (ms->Trace[p+maxPos] > 92) {
                            goto badtrace;
                        }
                    }
                    for (int p=4500; p<6000; p++) {
                        if (ms->Trace[p] > 120) {
                            maxPos = p;
                            goto afterfilter2;
                        }
                ...
                afterfilter2:
                    ...
                    for (int p=2000; p<4000; p++) {
                        if (ms->Trace[p+maxPos] < -100) {
                            goto badtrace;
                        }
                    }

                    for (int p=7100; p<8900; p++) {
                        if (ms->Trace[p+maxPos] > 120) {
                            goto badtrace;
                        }
                    }

                    for (int p=9700; p<12500; p++) {
                        if (ms->Trace[p+maxPos] > 97) {
                            goto badtrace;
                        }
                    }
                    ...
                    if (maxPos+off < 0) goto badtrace;

                    for (int p=1500; p<3000; p++) {
                        if (ms->Trace[maxPos+off+p] < -58) {
                            goto badtrace;
                        }
                    }

                    for (int p=16000; p<19000; p++) {
                        if (ms->Trace[maxPos+off+p] > 105) {
                            goto badtrace;
                        }
                    }
                    ...
                    //
//                    for (int p=10000; p<15000; p++) {
//                        if (ms->Trace[maxPos+off+p] < -85) {
//                            goto badtrace;
//                        }
//                    }
//
//                    off += 10500;
//
//                    if (maxPos+off < MOVE_RADIUS) goto badtrace;
//                    if (maxPos+off+MOVE_RADIUS > ms->NumberOfPoints) goto badtrace;
//
                    int off2 = 0;
//                    if (!meanTrace2) {
//                        meanLock.lock();
//                        if (!meanTrace2) {
//                            //first set a mean
//                            assure(meanTrace2 = (int8_t*)malloc((FALT_SIZE2 + MOVE_RADIUS) * sizeof(*meanTrace2)));
//                            memcpy(meanTrace2, (int8_t*)&ms->Trace[maxPos+off], (FALT_SIZE2 + MOVE_RADIUS) * sizeof(*meanTrace2));
//                        }
//                        meanLock.unlock();
//                    }else{
//                        off2 = maxfaltung(meanTrace2, &ms->Trace[maxPos+off], FALT_SIZE2);
//                    }
//
//                    off -= 10500;
//
...
//                        if (ms->Trace[maxPos+off+off2+p] < -55) {
//                            goto badtrace;
//                        }
//                    }
//
//
//                    if (maxPos+off+off2 < 0 || maxPos+off+off2+SAMPLE_SIZE > ms->NumberOfPoints) { //this should really not happen, because otherwise we're oob!
//                        goto badtrace;
//                    }

                                        
                    tracesInFile++;
                    tstarts->push_back({(int8_t*)&ms->Trace[maxPos+off+off2]/*Trace*/,(uint8_t*)ms->Input/*Plaintext*/,(uint8_t*)ms->Output/*Ciphertext*/});
