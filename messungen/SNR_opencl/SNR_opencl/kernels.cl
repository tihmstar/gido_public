...
#define G_SELECTOR_TYPE
#define G_SELECTOR_VALUE
#define G_SELECTOR_ROUND

//#define G_SELECTOR_TYPE SELECTOR_ROUND_HW
//#define G_SELECTOR_VALUE 8
//#define G_SELECTOR_ROUND 4

#define SELECTOR_PLAIN 0x100
#define SELECTOR_PLAIN_HW 0x101
#define SELECTOR_PLAIN_HW4 0x102
#define SELECTOR_PLAIN_KEY_HD 0x103
#define SELECTOR_SBOX 0x104
#define SELECTOR_SBOX_HW 0x105
#define SELECTOR_ZEROVALUE 0x106
#define SELECTOR_CIPHER 0x107
#define SELECTOR_CIPHER_HW 0x108
#define SELECTOR_CIPHER_HW4 0x109
#define SELECTOR_TTABLET0_HW4 0x10a
#define SELECTOR_TTABLET1_HW4 0x10b
#define SELECTOR_TTABLET2_HW4 0x10c
#define SELECTOR_TTABLET3_HW4 0x10d
#define SELECTOR_TTABLET0T1_HD4 0x10e
#define SELECTOR_TTABLET0T1T2_HD4 0x10f
#define SELECTOR_TTABLET0T1T2T3_HD4 0x110
#define SELECTOR_TTABLET0T1T2T3K1_HD4 0x111
#define SELECTOR_TTABLET0P0_HD4 0x112
#define SELECTOR_TTABLET0P0K0_HD4 0x113
#define SELECTOR_P0K0_HD4 0x114
#define SELECTOR_ROUND_HW 0x115
#define SELECTOR_ROUND_HW4 0x116
#define SELECTOR_ROUND_VALUE 0x117
inline uchar traceSelector(__global const struct MeasurementStructure *msm){
//    return msm->Plaintext[G_SELECTOR_VALUE]; //plaintext value
//    return hamming_weight(msm->Plaintext[G_SELECTOR_VALUE]); //plaintext value
//    return hamming_weight_32(*(__global uint*)(&msm->Plaintext[G_SELECTOR_VALUE]));


#if G_SELECTOR_TYPE == SELECTOR_PLAIN
    return msm->Plaintext[G_SELECTOR_VALUE]; //plaintext value
#endif

#if G_SELECTOR_TYPE == SELECTOR_PLAIN_HW
    return hamming_weight(msm->Plaintext[G_SELECTOR_VALUE]);
#endif

#if G_SELECTOR_TYPE == SELECTOR_PLAIN_HW4
    return hamming_weight_32(*(__global uint*)(&msm->Plaintext[G_SELECTOR_VALUE]));
#endif


#if G_SELECTOR_TYPE == SELECTOR_PLAIN_KEY_HD
    return hamming_weight(msm->Plaintext[G_SELECTOR_VALUE] ^ aeskey[16*14 + G_SELECTOR_VALUE]);
#endif

#if G_SELECTOR_TYPE == SELECTOR_SBOX
    return inv_sbox[msm->Plaintext[G_SELECTOR_VALUE] ^ aeskey[16*14 + G_SELECTOR_VALUE]]; //after sbox value
#endif

#if G_SELECTOR_TYPE == SELECTOR_SBOX_HW
    return hamming_weight(inv_sbox[msm->Plaintext[G_SELECTOR_VALUE] ^ aeskey[16*14 + G_SELECTOR_VALUE]]); //hamming sbox
#endif


#if G_SELECTOR_TYPE == SELECTOR_ZEROVALUE
    return (msm->Plaintext[G_SELECTOR_VALUE] ^ aeskey[16*14 + G_SELECTOR_VALUE]) == 0; //sbox input ZeroValue
#endif

#if G_SELECTOR_TYPE == SELECTOR_CIPHER
    return msm->Ciphertext[G_SELECTOR_VALUE];
#endif

#if G_SELECTOR_TYPE == SELECTOR_CIPHER_HW
    return hamming_weight(msm->Ciphertext[G_SELECTOR_VALUE]);
#endif

#if G_SELECTOR_TYPE == SELECTOR_CIPHER_HW4
    return hamming_weight_32(*(__global uint*)(&msm->Ciphertext[G_SELECTOR_VALUE]));
#endif

#if G_SELECTOR_TYPE == SELECTOR_TTABLET0_HW4
...
#if G_SELECTOR_TYPE == SELECTOR_TTABLET1_HW4
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    ptextAddkey[14] = msm->Plaintext[14] ^ aeskey[16*14 + 14];
    ptextAddkey[15] = msm->Plaintext[15] ^ aeskey[16*14 + 15];
    return hamming_weight_32(T1_inv[ptextAddkey[5]]); //hamming T1
#endif

#if G_SELECTOR_TYPE == SELECTOR_TTABLET2_HW4
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
#if G_SELECTOR_TYPE == SELECTOR_TTABLET3_HW4
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    ptextAddkey[14] = msm->Plaintext[14] ^ aeskey[16*14 + 14];
    ptextAddkey[15] = msm->Plaintext[15] ^ aeskey[16*14 + 15];
    return hamming_weight_32(T3_inv[ptextAddkey[15]]); //hamming T3
#endif


#if G_SELECTOR_TYPE == SELECTOR_TTABLET0T1_HD4
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    ptextAddkey[14] = msm->Plaintext[14] ^ aeskey[16*14 + 14];
    ptextAddkey[15] = msm->Plaintext[15] ^ aeskey[16*14 + 15];
    return hamming_weight_32(T0_inv[ptextAddkey[0]] ^ T1_inv[ptextAddkey[5]]);
#endif

#if G_SELECTOR_TYPE == SELECTOR_TTABLET0T1T2_HD4
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    ptextAddkey[14] = msm->Plaintext[14] ^ aeskey[16*14 + 14];
    ptextAddkey[15] = msm->Plaintext[15] ^ aeskey[16*14 + 15];
    return hamming_weight_32(T0_inv[ptextAddkey[0]] ^ T1_inv[ptextAddkey[5]] ^ T2_inv[ptextAddkey[10]]);
#endif

#if G_SELECTOR_TYPE == SELECTOR_TTABLET0T1T2T3_HD4
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    ptextAddkey[14] = msm->Plaintext[14] ^ aeskey[16*14 + 14];
    ptextAddkey[15] = msm->Plaintext[15] ^ aeskey[16*14 + 15];
    return hamming_weight_32(T0_inv[ptextAddkey[0]] ^ T1_inv[ptextAddkey[5]] ^ T2_inv[ptextAddkey[10]] ^ T3_inv[ptextAddkey[15]]);
#endif

#if G_SELECTOR_TYPE == SELECTOR_TTABLET0T1T2T3K1_HD4
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    ptextAddkey[14] = msm->Plaintext[14] ^ aeskey[16*14 + 14];
    ptextAddkey[15] = msm->Plaintext[15] ^ aeskey[16*14 + 15];
    return hamming_weight_32(T0_inv[ptextAddkey[0]] ^ T1_inv[ptextAddkey[5]] ^ T2_inv[ptextAddkey[10]] ^ T3_inv[ptextAddkey[15]] ^ *(__constant uint*)(&aeskey[16]));
#endif

#if G_SELECTOR_TYPE == SELECTOR_TTABLET0P0_HD4
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
#if G_SELECTOR_TYPE == SELECTOR_TTABLET0P0K0_HD4
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
#if G_SELECTOR_TYPE == SELECTOR_P0K0_HD4
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
#if G_SELECTOR_TYPE == SELECTOR_ROUND_HW
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
    ...
    for (int i=1; i<G_SELECTOR_ROUND; i++) { //4 middle rounds
        ...
        w[0] = T0_inv[ptextAddkey[ 0]] ^ T1_inv[ptextAddkey[ 5]] ^ T2_inv[ptextAddkey[10]] ^ T3_inv[ptextAddkey[15]] ^ *(__constant uint*)&aeskey[(14-i)*16 + 0 ];
        w[1] = T0_inv[ptextAddkey[ 4]] ^ T1_inv[ptextAddkey[ 9]] ^ T2_inv[ptextAddkey[14]] ^ T3_inv[ptextAddkey[ 3]] ^ *(__constant uint*)&aeskey[(14-i)*16 + 4 ];
        w[2] = T0_inv[ptextAddkey[ 8]] ^ T1_inv[ptextAddkey[13]] ^ T2_inv[ptextAddkey[ 2]] ^ T3_inv[ptextAddkey[ 7]] ^ *(__constant uint*)&aeskey[(14-i)*16 + 8 ];
        w[3] = T0_inv[ptextAddkey[12]] ^ T1_inv[ptextAddkey[ 1]] ^ T2_inv[ptextAddkey[ 6]] ^ T3_inv[ptextAddkey[11]] ^ *(__constant uint*)&aeskey[(14-i)*16 + 12];
...
#if G_SELECTOR_TYPE == SELECTOR_ROUND_HW4
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
#if G_SELECTOR_TYPE == SELECTOR_ROUND_VALUE
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
__kernel void computeMeans(__global const struct Tracefile *tr, __global double *curgroupsMean, __global ulong *curgroupsMeanCnt){
    ...
    for (uint s = 0; s<0x100; s++){
        if (meanCounts[s] == 0) continue;
        curgroupsMean[pointsPerTrace*s + i] /= meanCounts[s]; //divide by count
    }
  
    //only warp desync here
    if (i<0x100){ //let the first 0x100 threads write their curgroupsMeanCnt
        curgroupsMeanCnt[i] = meanCounts[i];
    }
}

__kernel void computeCenteredSums(__global const struct Tracefile *tr, __global const double *curgroupsMean, __global const ulong *curgroupsMeanCnt, __global double *curgroupsCenSum, __global ulong *curgroupsCenSumCnt){
    uint i = get_global_id(0) + get_global_id(1) * get_global_size(0);
    uint tracesInFile = tr->tracesInFile;
    uint pointsPerTrace = tr->ms->NumberOfPoints;
    if (i>=pointsPerTrace) return; //these guys are out of bounds
    ...
    ulong cenSumCounts[0x100] = {};//all threads compute a local cenSumCounts
...
__kernel void combineMeanAndCenteredSums(__global double *censumQ1, __global ulong *censumQ1cnt, __global const double *censumQ2, __global const ulong *censumQ2cnt, __global double *meanQ1, __global ulong *meanQ1cnt, __global const double *meanQ2, __global const ulong *meanQ2cnt, uint pointsPerTrace){
...
__kernel void mergeCoutners(__global ulong *censumQ1cnt, __global const ulong *censumQ2cnt, __global ulong *meanQ1cnt, __global const ulong *meanQ2cnt){
    uint s = get_global_id(0) + get_global_id(1) * get_global_size(0);
    if (s>=0x100) return; //these guys are out of bounds
