...
uint32_t computeMean(const Traces &tr, double *output, const trCollection &hypotMeanGuess){
    ...
    memset(output, 0, sizeof(double)*pointsPerTrace);
    ...
    for (uint32_t j=0; j<tracesInFile; j++) {
        ...
        for (uint16_t k = 0; k<0x100; k++) {
            hypotMeanGuess.vals[k] += ((msm->Plaintext[0] ^ k) == 0 ? 0 : 1) * factor;
        }
...
uint32_t computeCenteredSum(const Traces &tr, const double *mean, double *output, const trCollection &hypotMeanGuess, const trCollection &hypotCensumGuess, numberedTrace *curupperPart){
    ...
    double *hypotMeanGuessVals = hypotMeanGuess.vals;
    double *hypotCensumGuessVals = hypotCensumGuess.vals;
    double realTmp[pointsPerTrace];

    for (uint32_t j=0; j<tracesInFile; j++) {
        const Traces::MeasurementStructure *msm = tr[j];
        uint8_t plaintextbyte = msm->Plaintext[0];
        const int8_t *msmTrace = msm->Trace;
        
        for (uint32_t i=0; i<pointsPerTrace; i++) {
            double tmp = realTmp[i] = msmTrace[i] - mean[i];
            output[i] += tmp*tmp;
        }
        
        for (uint16_t k = 0; k<0x100; k++) {
            double hypotTmp = ((plaintextbyte ^ k) == 0 ? 0 : 1) - hypotMeanGuessVals[k];
            hypotCensumGuessVals[k] += hypotTmp*hypotTmp;
    
            double *curUpperTrace = curupperPart[k].trace;
            for (uint32_t i=0; i<pointsPerTrace; i++) {
