...
inline uchar hamming_weight_128(const uchar s[16]){
    return
    hamming_weight(s[ 0]) +
    hamming_weight(s[ 1]) +
    hamming_weight(s[ 2]) +
...
inline uchar powerModel(__global const struct MeasurementStructure *msm, uchar key){
    ...
    return hamming_weight_128(ptextAddkey);
}
...
#else
    if (k>=0x100) return; //these guys are out of bounds
#endif
...
__kernel void computeUpper(__global const struct Tracefile *tr, __global const double *mean, __global const double *meansHypot, __global double *upperPart){
    ...
    for (uint j=0; j < tracesInFile; j++,msm = (__global const struct MeasurementStructure *)(((__global uchar*)msm)+nextMsmOffset)){
        ...
        for (int k=0; k<0x100; k++){
            ...
            upperPart[k*pointsPerTrace + i] += hypotTmp*tmp;
        }
    }
}
