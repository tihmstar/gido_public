...
//__constant uchar aeskey[32] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F };
...
/* BEGIN BLOCK 0 */
inline uchar traceSelector_DEC_INPUT(__global const struct MeasurementStructure *msm){
    return msm->Plaintext[G_SELECTOR_VALUE];
}

inline uchar traceSelector_DEC_INPUT_HW(__global const struct MeasurementStructure *msm){
    return hamming_weight(msm->Plaintext[G_SELECTOR_VALUE]);
}

inline uchar traceSelector_DEC_INPUT_HW4(__global const struct MeasurementStructure *msm){
    return hamming_weight_32(*(__global uint*)(&msm->Plaintext[G_SELECTOR_VALUE]));
}

inline uchar traceSelector_DEC_INPUT_KEY_HW(__global const struct MeasurementStructure *msm){
    return hamming_weight(msm->Plaintext[G_SELECTOR_VALUE] ^ aeskey[16*14 + G_SELECTOR_VALUE]);
}

inline uchar traceSelector_SBOX(__global const struct MeasurementStructure *msm){
    return inv_sbox[msm->Plaintext[G_SELECTOR_VALUE] ^ aeskey[16*14 + G_SELECTOR_VALUE]];
}

inline uchar traceSelector_SBOX_HW(__global const struct MeasurementStructure *msm){
    return hamming_weight(inv_sbox[msm->Plaintext[G_SELECTOR_VALUE] ^ aeskey[16*14 + G_SELECTOR_VALUE]]); //hamming sbox
}

inline uchar traceSelector_ZEROVALUE(__global const struct MeasurementStructure *msm){
    return (msm->Plaintext[G_SELECTOR_VALUE] ^ aeskey[16*14 + G_SELECTOR_VALUE]) == 0; //sbox input ZeroValue
}

inline uchar traceSelector_DEC_OUTPUT(__global const struct MeasurementStructure *msm){
    return msm->Ciphertext[G_SELECTOR_VALUE];
}

inline uchar traceSelector_DEC_OUTPUT_HW(__global const struct MeasurementStructure *msm){
    return hamming_weight(msm->Ciphertext[G_SELECTOR_VALUE]);
}

inline uchar traceSelector_DEC_OUTPUT_HW4(__global const struct MeasurementStructure *msm){
    return hamming_weight_32(*(__global uint*)(&msm->Ciphertext[G_SELECTOR_VALUE]));
}
/* END BLOCK 0 */


inline uchar traceSelector_TTABLET0_HW4(__global const struct MeasurementStructure *msm){
...
inline uchar traceSelector_TTABLET1_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_TTABLET2_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_TTABLET3_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_TTABLET0T1_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_TTABLET0T1T2_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_TTABLET0T1T2T3_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_TTABLET0P0_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_TTABLET0P0K0_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_P0K0_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_ROUND_HW(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_ROUND_HW4(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_ROUND_VALUE(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_ROUND_SBOX(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_ROUND_SBOX_HW(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_ROUND_MC(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_ROUND_MC_HW(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_ROUND1_MC(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_ROUND1_MC_HW(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_ROUND1(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_ROUND1_SBOX(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_ROUND1_SBOX_HW(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_ROUND1_HW(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
inline uchar traceSelector_ROUND1_XOR_PLAIN(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
#define L_ROW (G_SELECTOR_VALUE % 4) //row == row_shift_amt
#define L_INV_NEW_INDEX ((G_SELECTOR_VALUE + 16 - L_ROW*4)%16)
    return ptextAddkey[G_SELECTOR_VALUE] ^ msm->Plaintext[L_INV_NEW_INDEX];
#undef L__INV_NEW_INDEX
#undef L_ROW
}

inline uchar traceSelector_ROUND1_XOR_PLAIN_HW(__global const struct MeasurementStructure *msm){
    uchar ptextAddkey[16];
    ptextAddkey[ 0] = msm->Plaintext[ 0] ^ aeskey[16*14 +  0];
    ptextAddkey[ 1] = msm->Plaintext[ 1] ^ aeskey[16*14 +  1];
    ptextAddkey[ 2] = msm->Plaintext[ 2] ^ aeskey[16*14 +  2];
...
#define L_ROW (G_SELECTOR_VALUE % 4) //row == row_shift_amt
#define L_INV_NEW_INDEX ((G_SELECTOR_VALUE + 16 - L_ROW*4)%16)
    return hamming_weight(ptextAddkey[G_SELECTOR_VALUE] ^ msm->Plaintext[L_INV_NEW_INDEX]);
#undef L_INV_NEW_INDEX
#undef L_ROW
}
