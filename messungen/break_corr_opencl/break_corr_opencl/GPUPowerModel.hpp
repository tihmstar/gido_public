...
class GPUPowerModel {
public:
    static const uint32_t _modelSize = 0x100;
...
public:
    GPUPowerModel(GPUDevice *parent, int deviceNum, cl_device_id device_id, cl_context context, std::string kernelsource, uint32_t point_per_trace, std::string modelname);
    ...
    void getResults(std::map<std::string,CPUModelResults*> &modelresults, bool isMidResult = false);
