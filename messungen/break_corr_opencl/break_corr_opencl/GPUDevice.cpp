...
//,_activeWorkersPossible(0), _activeWorkersRunning(0)
{
    ...
    cl_uint workersOnGPUCnt = 0;

    workersOnGPUCnt = (uint32_t)(_deviceMemSize / (fixedAllocDeviceMem + _deviceMemAlloced + (uint64_t)300e6)); //at least 200MB trace size
    
    //LIMIT here
#define WORKERS_LIMIT 4
    if (workersOnGPUCnt > WORKERS_LIMIT)
        workersOnGPUCnt = WORKERS_LIMIT;
        
    assure_(workersOnGPUCnt > 0);//we need at least one trace and worker

    printf("[D-%d] workersOnGPUCnt           =%u\n",_deviceNum,workersOnGPUCnt);

    _deviceMemAlloced = 0;
    for (auto &m : _gpuPowerModels) {
        m->spawnExecutionWorker(workersOnGPUCnt);
        _deviceMemAlloced += m->getDeviceMemAllocated();
    }
...
void GPUDevice::getResults(std::map<std::string,std::map<std::string,CPUModelResults*>> &modelresults, bool isMidResult){
    ...
    for (auto &m : _gpuPowerModels) {
        ...
        m->getResults(modelresults[m->getModelName()], isMidResult);
    }
