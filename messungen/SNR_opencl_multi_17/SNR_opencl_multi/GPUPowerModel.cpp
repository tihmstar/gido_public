...
: _deviceNum(deviceNum), _deviceMemAlloced(0), _point_per_trace(points_per_trace), _modelSize(modelSize), _modelname(modelname), _context(cntx), _prevWaitEvent(NULL)
{
    ...
    if (modelname.substr(0,strlen("traceSelector")) != "traceSelector") {
        ssize_t firstUnderscore = modelname.find("_");
        assure_(firstUnderscore != std::string::npos);
        std::string selstr = modelname.substr(0,firstUnderscore);
        modelname = modelname.substr(firstUnderscore+1);
        
        std::string findstr = "#define G_SELECTOR_VALUE";
        std::string replacestr = "#define G_SELECTOR_VALUE " + selstr + " //";
        
        ssize_t findpos = kernelsource.find(findstr);
        assure_(findpos != std::string::npos);

        kernelsource = kernelsource.replace(findpos, findstr.size(), replacestr);
    }
